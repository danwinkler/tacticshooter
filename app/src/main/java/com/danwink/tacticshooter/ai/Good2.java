...
public class Good2 extends ComputerPlayer {
	...
	public void setValues() {
		...
		// Point ownership
		// The idea is that open points should be the priority, and then try to take
		// enemy points.
	...
	public void update(PathFinder finder) {
		// BUY UNITS
		// TODO: figure out better way to choose what kind of unit to buy, and when to
		// buy it
		if (player.money > 20) {
		...
		for (Building b : l.buildings) {
			...
			if (b.t == null) {
				score += weightBuildingOwnerNotTaken;
				...
				score += weightBuildingOwnerEnemy;
			}
			...
			// Add weights based on shortest distance to point owned by team (shorter is
			// better)
			...
			for (Building bu : l.buildings) {
				if (bu.t != null && bu.t.id == this.player.team.id && bu.hold == Building.HOLDMAX) {
					Path p = finder.findPath(null, l.getTileX(bu.x), l.getTileY(bu.y), l.getTileX(b.x),
							l.getTileY(b.y));
					if (p == null)
						continue;
		...
		for (Unit u : units) {
			...
			for (Building bu : l.buildings) {
				if (bu.t != null && bu.t.id == this.player.team.id && bu.hold == Building.HOLDMAX) {
					Path p = finder.findPath(null, l.getTileX(bu.x), l.getTileY(bu.y), l.getTileX(u.x),
							l.getTileY(u.y));
					if (p == null)
						continue;
					float d2 = p.getLength();
			...
			Path p = finder.findPath(null, l.getTileX(targetBuilding.x), l.getTileY(targetBuilding.y), l.getTileX(u.x),
					l.getTileY(u.y));
			if (p != null) {
		...
		ci.sl.received(fc, new Message(MessageType.SETATTACKPOINT, new Object[] {
				new Point2i(targetBuilding.x / Level.tileSize, targetBuilding.y / Level.tileSize), unitIdsToMove }));
	}
}
